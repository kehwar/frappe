...
class TestPermissions(FrappeTestCase):
	...
	def test_permission_query_conditions_for_read(self):
		"""Test that permission_query_conditions are checked for read and select actions"""
		# Test with Event doctype which has permission_query_conditions
		# Event.get_permission_query_conditions returns a condition that only allows
		# public events or events owned by the user
		
		frappe.set_user("Administrator")
		
		# Create a public event
		public_event = frappe.get_doc({
			"doctype": "Event",
			"subject": "Test Public Event",
			"event_type": "Public",
			"starts_on": frappe.utils.now_datetime()
		}).insert(ignore_permissions=True)
		
		# Create a private event owned by test1@example.com
		private_event_test1 = frappe.get_doc({
			"doctype": "Event",
			"subject": "Test Private Event Test1",
			"event_type": "Private",
			"starts_on": frappe.utils.now_datetime(),
			"owner": "test1@example.com"
		})
		private_event_test1.owner = "test1@example.com"
		private_event_test1.insert(ignore_permissions=True)
		
		# Create a private event owned by test2@example.com
		private_event_test2 = frappe.get_doc({
			"doctype": "Event",
			"subject": "Test Private Event Test2",
			"event_type": "Private",
			"starts_on": frappe.utils.now_datetime(),
			"owner": "test2@example.com"
		})
		private_event_test2.owner = "test2@example.com"
		private_event_test2.insert(ignore_permissions=True)
		
		try:
			# Test as test1@example.com
			frappe.set_user("test1@example.com")
			
			# Should have access to public event
			self.assertTrue(frappe.has_permission("Event", "read", public_event))
			
			# Should have access to own private event
			self.assertTrue(frappe.has_permission("Event", "read", private_event_test1))
			
			# Should NOT have access to other user's private event
			self.assertFalse(frappe.has_permission("Event", "read", private_event_test2))
			
			# Test select permission as well
			self.assertTrue(frappe.has_permission("Event", "select", public_event))
			self.assertTrue(frappe.has_permission("Event", "select", private_event_test1))
			self.assertFalse(frappe.has_permission("Event", "select", private_event_test2))
			
			# Test as test2@example.com
			frappe.set_user("test2@example.com")
			
			# Should have access to public event
			self.assertTrue(frappe.has_permission("Event", "read", public_event))
			
			# Should NOT have access to test1's private event
			self.assertFalse(frappe.has_permission("Event", "read", private_event_test1))
			
			# Should have access to own private event
			self.assertTrue(frappe.has_permission("Event", "read", private_event_test2))
			
		finally:
			# Cleanup
			frappe.set_user("Administrator")
			frappe.delete_doc("Event", public_event.name, force=True)
			frappe.delete_doc("Event", private_event_test1.name, force=True)
			frappe.delete_doc("Event", private_event_test2.name, force=True)
