...
class TestSafeExec(FrappeTestCase):
	...
	def test_safe_exec_globals_hook(self):
		...
		try:
			...
			# Cleanup
			if hasattr(test_module, "test_safe_exec_globals_hook_fn"):
				delattr(test_module, "test_safe_exec_globals_hook_fn")
			frappe.local.conf["safe_exec_globals"] = original_hooks
	...
	def test_safe_eval_globals_hook(self):
		...
		try:
			...
			# Cleanup
			if hasattr(test_module, "test_safe_eval_globals_hook_fn"):
				delattr(test_module, "test_safe_eval_globals_hook_fn")
			frappe.local.conf["safe_eval_globals"] = original_hooks
